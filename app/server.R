...
# ------------------------- Introduction ---------------------------
  ...
  Event_filter <- reactive({
    ...
    filter(anime, Genres %in% input$GenreFinder) %>%
      filter(Type %in% input$TypeFinder) %>%
      group_by(Genres, Type) %>%
      mutate(mean_Score = mean(as.numeric(Score))) %>%
      ungroup()
  ...
  output$plot1 <- renderPlot({
    ...
    input$TypeFinder
      ...
      ggplot(data = Event_filter(),aes(y = mean_Score, x = Type, fill = Genres))+
        geom_bar(stat = "summary", position = "dodge", fun = "mean")+
        ggtitle("Mean Score in Each Type")+
